...
async function main() {
  ...
  report += `| Task | ops/sec | Margin of Error |\n`;
  report += `|---|---|---|\n`;
  b1.tasks.forEach(t => {
    report += `| ${t.name} | ${t.result?.hz.toFixed(2)} | ±${t.result?.moe.toFixed(2)}% |\n`;
  });
  ...
  fs.writeFileSync(path.resolve('benchmarks/results/latest.md'), report);
  console.log(`📄 Saved to benchmarks/results/latest.md`);
}
